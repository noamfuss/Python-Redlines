...
- **ClippitEngine** — wraps Clippit WmlComparer (original engine)
...
1. **Python layer** (`src/python_redlines/engines.py`):
   ...
   - `ClippitEngine(BaseEngine)` — sets constants for the Clippit binary (`dist/`, `bin/`, `redlines`)
   - `DocxodusEngine(BaseEngine)` — sets constants for the Docxodus binary (`dist_docxodus/`, `bin_docxodus/`, `redline`)

   Both engines expose `run_redline(author_tag, original, modified, **kwargs)`. `DocxodusEngine` overrides `_build_command()` to translate kwargs (e.g. `detect_moves`, `detail_threshold`) into CLI flags for the Docxodus binary. `ClippitEngine` uses the legacy 4-positional-arg format and ignores kwargs.

2. **C# binaries**:
   - `csproj/Program.cs` — Clippit CLI tool
...
- `src/python_redlines/engines.py` — BaseEngine, ClippitEngine, and DocxodusEngine classes
...
- `csproj/Program.cs` — Clippit C# comparison utility
...
Tests must be run from the project root (fixtures use relative paths like `tests/fixtures/original.docx`). The ClippitEngine integration test validates that comparing the fixture documents produces exactly 9 revisions. Docxodus uses a different stdout format (`"revision(s) found"` vs `"Revisions found: 9"`).
